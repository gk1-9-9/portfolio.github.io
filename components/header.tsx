    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/10 backdrop-blur-md dark:bg-navy/10 shadow-sm' : 'bg-transparent'}`}>
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          ...
          >
            <Link href="/" className={`text-2xl font-bold tracking-wider ${theme === "light" ? "text-black" : "text-white"}`}>
              <span className="bg-gradient-to-r from-gold to-gold/70 bg-clip-text text-transparent">GK</span>
            </Link>
          </motion.div>
          ...
          <div className="flex items-center space-x-4">
            <Button
              ...
              className={`${theme === "light" ? "text-black" : "text-white/90"} hover:text-gold hover:bg-transparent`}
            >
              {theme === "dark" ? (
            ...
            >
              {isMobileMenuOpen ? (
                <X className={`${theme === "light" ? "text-black" : "text-white/90"} h-6 w-6`} />
              ) : (
                <Menu className={`${theme === "light" ? "text-black" : "text-white/90"} h-6 w-6`} />
              )}
                ...
                <Link
                  ...
                  className={`flex items-center gap-2 p-2 rounded-md transition-colors ${
                    pathname === item.href
                      ...
                      : theme === "light"
                         ? 'text-black hover:bg-white/5 hover:text-gold'
